...
#         preds += 2*(self.user_b(user) + self.item_b(item))
...
#             loss = self.loss_c(pred_c, label_list,
#                                self.inverse_propensity_c, item_list)
...
#             loss = self.loss_t(pred_t, label_list,
#                                 self.inverse_propensity_t, item_list)

#             loss_reg_c = self.reg_uc * torch.norm(user_embedding, 2)  # 改
#             loss_reg_c += self.reg_ic * torch.norm(item_embedding_c, 2)  # 改
#             loss_reg_t = self.reg_ut * torch.norm(user_embedding, 1)  # 改
#             loss_reg_t += self.reg_it * torch.norm(item_embedding_t, 1)  # 改
...
class CausE(nn.Module):
    ...
    def forward(self, user, item, is_train=0):
        ...
        if is_train == 0:
            weight = torch.Tensor(
                list(map(lambda x: pop_item[item_list_[x]],
                        range(0, len(item0))))).to(opt.device)
        elif is_train == 1:
            
            weight = torch.Tensor(
                list(map(lambda x: pop_item[item_list_[x]] + 10 * np.abs(
                    pop_item_val[item_list_[x]] - pop_item[item_list_[x]]),
                        range(0, len(item0))))).to(opt.device)
        else:
            weight = torch.Tensor(
                list(map(lambda x: pop_item[item_list_[x]] + 10 * np.abs(
                    pop_item_tst[item_list_[x]] - pop_item[item_list_[x]]),
                         range(0, len(item0))))).to(opt.device)
        ...
        if control == True:
            ...
            for i in range(len(dot_c)):
                if dot_c[i] <= 0:
                    ...
                    dot_c[i] = dot_c[i] + 1
            # weight = torch.Tensor(list(map(lambda x: (pop_item[item_list_[x]]),
            #                         range(0, len(item0))))).to(opt.device)
            # weight = np.power(weight, opt.pop_exp)
            ...
            # weight = torch.Tensor(list(map(lambda x: (pop_item[item_list_[x]]),
            #                         range(0, len(item0))))).to(opt.device)
            # weight = np.power(weight, 0.1)
            # dot_t_pop = dot_t * weight.unsqueeze(dim=1)
            # dot_t_pop = torch.log(torch.sigmoid(dot_t_pop))
            pred_t = dot_t + self.user_b(user_list) + self.item_b(item_list)
            pred_t = pred_t.squeeze()
    ...
    def predict(self, user, item, is_train):
        return self.forward(user, item, is_train)
